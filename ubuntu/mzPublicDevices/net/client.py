...
class TimerFactory(ClientFactory):  
	...
	def __init__(self,dename,mzcall=False):  
		self.number_of_connections = 0
		self.mzcall=mzcall
		self.registinfo=json.dumps({"name":dename})
class Client(object):
	...
	def run(self):
		...
		reactor.connectTCP(self.host,int(self.appPort), TimerFactory(self.name,self.mzcall))  
		reactor.run()
